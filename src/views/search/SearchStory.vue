  ...
  <div class="min-h-screen bg-[#f1f5f9] dark:bg-slate-900">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="mb-8 p-8 bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700">
        <h1 class="text-3xl font-black text-slate-800 dark:text-slate-100 mb-2 tracking-tighter">
          Kết quả tìm kiếm: "{{ keyword }}"
        </h1>
        <p v-if="!loading && totalRecords > 0" class="text-slate-500 dark:text-slate-400 font-medium">
          Tìm thấy <span class="text-indigo-600 font-black">{{ totalRecords }}</span> truyện
        </p>
      </div>
      
      <div v-if="!keyword" class="text-center py-32 bg-white dark:bg-slate-800 rounded-3xl border border-dashed border-slate-200 dark:border-slate-700">
        <i class="pi pi-search text-6xl text-slate-200 mb-4"></i>
        <p class="text-slate-400 dark:text-slate-500 font-medium">Nhập từ khóa để tìm kiếm truyện</p>
      </div>
      
      <div v-else-if="stories.length === 0" class="text-center py-32 bg-white dark:bg-slate-800 rounded-3xl border border-dashed border-slate-200 dark:border-slate-700">
        <i class="pi pi-search text-6xl text-slate-200 mb-4"></i>
        <h2 class="text-xl font-black text-slate-800 dark:text-slate-100 mb-2">
